...
class BotsPage extends Component {
    ...
    componentDidMount() {
        ...
        fetch('https://timothykagondu.github.io/Botdb/db.json')
      ...
      removeFromArmy = (bot) => {
        ...
        removeBotPermanently = (bot) => {
            const newCollection = this.state.botCollection.filter((card) => card !== bot);
            const newFilteredCollection = this.state.filteredCollection.filter((card) => card !== bot);
            const newArmy = this.state.botArmy.filter((card) => card !== bot);
          
            this.setState(
              { botCollection: newCollection, filteredCollection: newFilteredCollection, botArmy: newArmy },
              () => {
      ...
      render() {
        ...
        return (
            <div>
              ...
              <BotArmy bots={botArmy} action={this.removeFromArmy} removeCard={this.removeBotPermanently} /> {collectionVisible ? < BotCollection botCollection={filteredCollection}

                  action={this.displayBotSpecs} removeCard={this.removeBotPermanently} /> : < BotSpecs bot={botSpecs} back={this.displayBotCollection} enlist={this.addToArmy} />
              }
            </div>
          )
        }
